package main.java.io.nshusa.rsam.graphics.render;
	...
	public static void drawShadedTriangle(int y_a, int y_b, int y_c, int x_a, int x_b, int x_c, int hsl1, int hsl2, int hsl3, float z_a, float z_b, float z_c) {
		...
		if (y_a <= y_b && y_a <= y_c) {
			...
			if (y_b < y_c) {
				...
				if (y_a != y_b && c_to_a < a_to_b || y_a == y_b && c_to_a > b_to_c) {
					y_c -= y_b;
					y_b -= y_a;
					for (y_a = scanOffsets[y_a]; --y_b >= 0; y_a += width) {
						drawShadedScanline(pixels, y_a, x_c >> 16, x_a >> 16, r3, g3, b3, r1, g1, b1, z_a,
								depth_slope);
						x_c += c_to_a;
						x_a += a_to_b;
						r3 += dr3;
					...
					while (--y_c >= 0) {
						drawShadedScanline(pixels, y_a, x_c >> 16, x_b >> 16, r3, g3, b3, r2, g2, b2, z_a,
								depth_slope);
						x_c += c_to_a;
						x_b += b_to_c;
						r3 += dr3;
				...
				y_b -= y_a;
				for (y_a = scanOffsets[y_a]; --y_b >= 0; y_a += width) {
					drawShadedScanline(pixels, y_a, x_a >> 16, x_c >> 16, r1, g1, b1, r3, g3, b3, z_a,
							depth_slope);
					x_c += c_to_a;
					x_a += a_to_b;
					r3 += dr3;
				...
				while (--y_c >= 0) {
					drawShadedScanline(pixels, y_a, x_b >> 16, x_c >> 16, r2, g2, b2, r3, g3, b3, z_a,
							depth_slope);
					x_c += c_to_a;
					x_b += b_to_c;
					r3 += dr3;
			...
			if (y_a != y_c && c_to_a < a_to_b || y_a == y_c && b_to_c > a_to_b) {
				y_b -= y_c;
				y_c -= y_a;
				for (y_a = scanOffsets[y_a]; --y_c >= 0; y_a += width) {
					drawShadedScanline(pixels, y_a, x_b >> 16, x_a >> 16, r2, g2, b2, r1, g1, b1, z_a,
							depth_slope);
					x_b += c_to_a;
					x_a += a_to_b;
					r2 += dr3;
		...
		if (y_b <= y_c) {
			...
			if (y_c < y_a) {
				...
				if (y_b != y_c && a_to_b < b_to_c || y_b == y_c && a_to_b > c_to_a) {
					y_a -= y_c;
					y_c -= y_b;
					for (y_b = scanOffsets[y_b]; --y_c >= 0; y_b += width) {
						drawShadedScanline(pixels, y_b, x_a >> 16, x_b >> 16, r1, g1, b1, r2, g2, b2, z_b,
								depth_slope);
						x_a += a_to_b;
						x_b += b_to_c;
						r1 += dr1;
					...
					while (--y_a >= 0) {
						drawShadedScanline(pixels, y_b, x_a >> 16, x_c >> 16, r1, g1, b1, r3, g3, b3, z_b,
								depth_slope);
						x_a += a_to_b;
						x_c += c_to_a;
						r1 += dr1;
				...
				y_c -= y_b;
				for (y_b = scanOffsets[y_b]; --y_c >= 0; y_b += width) {
					drawShadedScanline(pixels, y_b, x_b >> 16, x_a >> 16, r2, g2, b2, r1, g1, b1, z_b,
							depth_slope);
					x_a += a_to_b;
					x_b += b_to_c;
					r1 += dr1;
				...
				while (--y_a >= 0) {
					drawShadedScanline(pixels, y_b, x_c >> 16, x_a >> 16, r3, g3, b3, r1, g1, b1, z_b,
							depth_slope);
					x_a += a_to_b;
					x_c += c_to_a;
					r1 += dr1;
			...
			if (a_to_b < b_to_c) {
				y_c -= y_a;
				y_a -= y_b;
				for (y_b = scanOffsets[y_b]; --y_a >= 0; y_b += width) {
					drawShadedScanline(pixels, y_b, x_c >> 16, x_b >> 16, r3, g3, b3, r2, g2, b2, z_b,
							depth_slope);
					x_c += a_to_b;
					x_b += b_to_c;
					r3 += dr1;
		...
		if (y_a < y_b) {
			...
			if (b_to_c < c_to_a) {
				y_b -= y_a;
				y_a -= y_c;
				for (y_c = scanOffsets[y_c]; --y_a >= 0; y_c += width) {
					drawShadedScanline(pixels, y_c, x_b >> 16, x_c >> 16, r2, g2, b2, r3, g3, b3, z_c,
							depth_slope);
					x_b += b_to_c;
					x_c += c_to_a;
					r2 += dr2;
				...
				while (--y_b >= 0) {
					drawShadedScanline(pixels, y_c, x_b >> 16, x_a >> 16, r2, g2, b2, r1, g1, b1, z_c,
							depth_slope);
					x_b += b_to_c;
					x_a += a_to_b;
					r2 += dr2;
		...
		if (b_to_c < c_to_a) {
			y_a -= y_b;
			y_b -= y_c;
			for (y_c = scanOffsets[y_c]; --y_b >= 0; y_c += width) {
	...
	public static void drawTexturedTriangle(int y_a, int y_b, int y_c, int x_a, int x_b, int x_c, int k1, int l1,
		...
		if (y_a <= y_b && y_a <= y_c) {
			...
			if (y_b < y_c) {
				...
				if (y_a != y_b && c_to_a < a_to_b || y_a == y_b && c_to_a > b_to_c) {
					...
					y_a = scanOffsets[y_a];
					while (--y_b >= 0) {
						drawTexturedScanline(pixels, texture, y_a, x_c >> 16, x_a >> 16, i2 >> 8, k1 >> 8, Oa,
					...
					while (--y_c >= 0) {
						drawTexturedScanline(pixels, texture, y_a, x_c >> 16, x_b >> 16, i2 >> 8, l1 >> 8, Oa,
								Ob, Oc, Ha, Hb, Hc, z_a, depth_slope);
						x_c += c_to_a;
						x_b += b_to_c;
						z_a += depth_increment;
				...
				y_a = scanOffsets[y_a];
				while (--y_b >= 0) {
					drawTexturedScanline(pixels, texture, y_a, x_a >> 16, x_c >> 16, k1 >> 8, i2 >> 8, Oa, Ob,
				...
				while (--y_c >= 0) {
					drawTexturedScanline(pixels, texture, y_a, x_b >> 16, x_c >> 16, l1 >> 8, i2 >> 8, Oa, Ob,
							Oc, Ha, Hb, Hc, z_a, depth_slope);
					x_c += c_to_a;
					x_b += b_to_c;
					z_a += depth_increment;
			...
			if (y_a != y_c && c_to_a < a_to_b || y_a == y_c && b_to_c > a_to_b) {
				...
				y_a = scanOffsets[y_a];
				while (--y_c >= 0) {
					drawTexturedScanline(pixels, texture, y_a, x_b >> 16, x_a >> 16, l1 >> 8, k1 >> 8, Oa, Ob,
							Oc, Ha, Hb, Hc, z_a, depth_slope);
					x_b += c_to_a;
					x_a += a_to_b;
					l1 += grad_c_off;
				...
				while (--y_b >= 0) {
					drawTexturedScanline(pixels, texture, y_a, x_c >> 16, x_a >> 16, i2 >> 8, k1 >> 8, Oa, Ob,
							Oc, Ha, Hb, Hc, z_a, depth_slope);
					x_c += b_to_c;
					x_a += a_to_b;
					i2 += grad_b_off;
			...
			y_a = scanOffsets[y_a];
			while (--y_c >= 0) {
				drawTexturedScanline(pixels, texture, y_a, x_a >> 16, x_b >> 16, k1 >> 8, l1 >> 8, Oa, Ob, Oc,
			...
			while (--y_b >= 0) {
				drawTexturedScanline(pixels, texture, y_a, x_a >> 16, x_c >> 16, k1 >> 8, i2 >> 8, Oa, Ob, Oc,
						Ha, Hb, Hc, z_a, depth_slope);
				x_c += b_to_c;
				x_a += a_to_b;
				i2 += grad_b_off;
		...
		if (y_b <= y_c) {
			...
			if (y_c < y_a) {
				...
				if (y_b != y_c && a_to_b < b_to_c || y_b == y_c && a_to_b > c_to_a) {
					...
					y_b = scanOffsets[y_b];
					while (--y_c >= 0) {
						drawTexturedScanline(pixels, texture, y_b, x_a >> 16, x_b >> 16, k1 >> 8, l1 >> 8, Oa,
					...
					while (--y_a >= 0) {
						drawTexturedScanline(pixels, texture, y_b, x_a >> 16, x_c >> 16, k1 >> 8, i2 >> 8, Oa,
								Ob, Oc, Ha, Hb, Hc, z_b, depth_slope);
						x_a += a_to_b;
						x_c += c_to_a;
						k1 += grad_a_off;
				...
				y_b = scanOffsets[y_b];
				while (--y_c >= 0) {
					drawTexturedScanline(pixels, texture, y_b, x_b >> 16, x_a >> 16, l1 >> 8, k1 >> 8, Oa, Ob,
				...
				while (--y_a >= 0) {
					drawTexturedScanline(pixels, texture, y_b, x_c >> 16, x_a >> 16, i2 >> 8, k1 >> 8, Oa, Ob,
							Oc, Ha, Hb, Hc, z_b, depth_slope);
					x_a += a_to_b;
					x_c += c_to_a;
					k1 += grad_a_off;
			...
			if (a_to_b < b_to_c) {
				...
				y_b = scanOffsets[y_b];
				while (--y_a >= 0) {
					drawTexturedScanline(pixels, texture, y_b, x_c >> 16, x_b >> 16, i2 >> 8, l1 >> 8, Oa, Ob,
							Oc, Ha, Hb, Hc, z_b, depth_slope);
					x_c += a_to_b;
					x_b += b_to_c;
					i2 += grad_a_off;
				...
				while (--y_c >= 0) {
					drawTexturedScanline(pixels, texture, y_b, x_a >> 16, x_b >> 16, k1 >> 8, l1 >> 8, Oa, Ob,
							Oc, Ha, Hb, Hc, z_b, depth_slope);
					x_a += c_to_a;
					x_b += b_to_c;
					k1 += grad_c_off;
			...
			y_b = scanOffsets[y_b];
			while (--y_a >= 0) {
				drawTexturedScanline(pixels, texture, y_b, x_b >> 16, x_c >> 16, l1 >> 8, i2 >> 8, Oa, Ob, Oc,
			...
			while (--y_c >= 0) {
				drawTexturedScanline(pixels, texture, y_b, x_b >> 16, x_a >> 16, l1 >> 8, k1 >> 8, Oa, Ob, Oc,
						Ha, Hb, Hc, z_b, depth_slope);
				x_a += c_to_a;
				x_b += b_to_c;
				k1 += grad_c_off;
		...
		if (y_a < y_b) {
			...
			if (b_to_c < c_to_a) {
				...
				y_c = scanOffsets[y_c];
				while (--y_a >= 0) {
					drawTexturedScanline(pixels, texture, y_c, x_b >> 16, x_c >> 16, l1 >> 8, i2 >> 8, Oa, Ob,
				...
				while (--y_b >= 0) {
					drawTexturedScanline(pixels, texture, y_c, x_b >> 16, x_a >> 16, l1 >> 8, k1 >> 8, Oa, Ob,
							Oc, Ha, Hb, Hc, z_c, depth_slope);
					x_b += b_to_c;
					x_a += a_to_b;
					l1 += grad_b_off;
			...
			y_c = scanOffsets[y_c];
			while (--y_a >= 0) {
				drawTexturedScanline(pixels, texture, y_c, x_c >> 16, x_b >> 16, i2 >> 8, l1 >> 8, Oa, Ob, Oc,
			...
			while (--y_b >= 0) {
				drawTexturedScanline(pixels, texture, y_c, x_a >> 16, x_b >> 16, k1 >> 8, l1 >> 8, Oa, Ob, Oc,
						Ha, Hb, Hc, z_c, depth_slope);
				x_b += b_to_c;
				x_a += a_to_b;
				l1 += grad_b_off;
		...
		if (b_to_c < c_to_a) {
			...
			y_c = scanOffsets[y_c];
			while (--y_b >= 0) {
				drawTexturedScanline(pixels, texture, y_c, x_a >> 16, x_c >> 16, k1 >> 8, i2 >> 8, Oa, Ob, Oc,
						Ha, Hb, Hc, z_c, depth_slope);
				x_a += b_to_c;
				x_c += c_to_a;
				k1 += grad_b_off;
			...
			while (--y_a >= 0) {
				drawTexturedScanline(pixels, texture, y_c, x_b >> 16, x_c >> 16, l1 >> 8, i2 >> 8, Oa, Ob, Oc,
						Ha, Hb, Hc, z_c, depth_slope);
				x_b += a_to_b;
				x_c += c_to_a;
				l1 += grad_a_off;
		...
		y_c = scanOffsets[y_c];
		while (--y_b >= 0) {
			drawTexturedScanline(pixels, texture, y_c, x_c >> 16, x_a >> 16, i2 >> 8, k1 >> 8, Oa, Ob, Oc, Ha,
		...
		while (--y_a >= 0) {
			drawTexturedScanline(pixels, texture, y_c, x_c >> 16, x_b >> 16, i2 >> 8, l1 >> 8, Oa, Ob, Oc, Ha,
					Hb, Hc, z_c, depth_slope);
			x_b += a_to_b;
			x_c += c_to_a;
			l1 += grad_a_off;
