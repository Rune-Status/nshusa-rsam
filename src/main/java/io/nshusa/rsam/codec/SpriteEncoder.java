...
import main.java.io.nshusa.rsam.util.ColorQuantizer;
    ...
    public static void encode(File dir, int encodingType) {
        ...
        try(DataOutputStream idxOut = new DataOutputStream(ibos)) {
            ...
            for (File imageArchiveDir : dir.listFiles()) {
                ...
                try(FileOutputStream dFos = new FileOutputStream(new File(outputDir, imageArchiveDir.getName() + ".dat"))) {
                    dFos.write(dbos.toByteArray());
                }

            }

            try(FileOutputStream iFos = new FileOutputStream(new File(outputDir, "index.dat"))) {
                iFos.write(ibos.toByteArray());
            }
